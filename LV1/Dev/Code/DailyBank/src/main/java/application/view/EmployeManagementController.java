...
public class EmployeManagementController {
	// Etat courant de l'application
		...
		private void doRechercher() {
			...
			listeCli = this.emDialogController.getlisteComptes(numCompte, debutNom, debutPrenom);
		...
		private void doComptesClient() {
			...
			if (selectedIndice >= 0) {
				...
				this.emDialogController.gererComptesClient(client);
			}
		}
		...
		private void doModifierClient() {
			...
			if (selectedIndice >= 0) {
				...
				Client result = this.emDialogController.modifierClient(cliMod);
				if (result != null) {
		...
		private void doNouveauClient() {
			Client client;
			client = this.emDialogController.nouveauClient();
			if (client != null) {
